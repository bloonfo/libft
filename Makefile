# **************************************************************************** #
#                                                                              #
#                                                         :::      ::::::::    #
#    Makefile                                           :+:      :+:    :+:    #
#                                                     +:+ +:+         +:+      #
#    By: matgalla <matgalla@student.42.fr>          +#+  +:+       +#+         #
#                                                 +#+#+#+#+#+   +#+            #
#    Created: 2024/10/15 15:14:10 by matgalla          #+#    #+#              #
#    Updated: 2024/10/28 14:55:49 by matgalla         ###   ########.fr        #
#                                                                              #
# **************************************************************************** #

NAME		= libft.a

INCLUDES	= includes

SRCS		= ft_atoi.c ft_bzero.c ft_calloc.c ft_isalnum.c ft_isalpha.c \
			  ft_isascii.c ft_isdigit.c ft_isprint.c ft_memchr.c ft_memcpy.c \
			  ft_memmove.c ft_memset.c ft_strchr.c ft_strdup.c ft_strlcat.c \
			  ft_strlcpy.c ft_strlen.c ft_strncmp.c ft_strnstr.c ft_strrchr.c \
			  ft_tolower.c ft_toupper.c ft_putchar_fd.c ft_putstr_fd.c ft_putendl_fd.c \
			  ft_putnbr_fd.c ft_memcmp.c ft_substr.c ft_strtrim.c ft_itoa.c \
			  ft_strjoin.c ft_split.c ft_strmapi.c ft_striteri.c

SRCS_BONUS  = ft_lstnew_bonus.c ft_lstadd_front_bonus.c ft_lstsize_bonus.c ft_lstlast_bonus.c \
			  ft_lstadd_back_bonus.c ft_lstdelone_bonus.c ft_lstclear_bonus.c ft_lstiter_bonus.c \
			  ft_lstmap_bonus.c

OBJS		= $(SRCS:.c=.o)

OBJS_BONUS	= $(SRCS_BONUS:.c=.o)

CC			= cc
CFLAGS		= -Wall -Wextra -Werror

# rule to make the object files
%.o: %.c
		$(CC) $(CFLAGS) -I $(INCLUDES) -c $< -o $@

all: $(NAME)

# compile and make the executable
$(NAME): $(OBJS)
	ar rcs $(NAME) $(OBJS)

bonus: $(OBJS) $(OBJS_BONUS)
	ar rcs $(NAME) $(OBJS) $(OBJS_BONUS)

# clean object files
clean:
		rm -f $(OBJS)

# clean object files and executable
fclean: clean
		rm -f $(NAME)

re: fclean all

# tells the Makefile that these targets are commands and not files
.PHONY: all clean fclean re
